...
public class AdminDashboardService {
    ...
    private long getActiveUsersCount() {
        ...
        return userActivityRepository.countActivitiesByTypeSince("login", oneDayAgo);
    }
    
    private long getNewUsersToday() {
        LocalDateTime startOfDay = LocalDateTime.now().withHour(0).withMinute(0).withSecond(0);
        return userActivityRepository.countActivitiesByTypeSince("user_registration", startOfDay);
    }
    
    private long getSuccessfulLoginsToday() {
        LocalDateTime startOfDay = LocalDateTime.now().withHour(0).withMinute(0).withSecond(0);
        return userActivityRepository.countActivitiesByTypeSince("login", startOfDay);
    }
    
    private double getAverageCompletionRate() {
        Double avgCompletion = profileCompletionRepository.getAverageCompletionPercentage();
        return avgCompletion != null ? avgCompletion : 0.0;
    }
    ...
    private AdminDashboardStatsResponse.AdminRecentActivity mapActivityToAdminResponse(UserActivity activity) {
        String message = getAdminActivityMessage(activity.getActivityType(), activity.getUserId());
    ...
    private String getAdminActivityMessage(String activityType, Long userId) {
